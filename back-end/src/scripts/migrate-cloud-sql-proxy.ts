...
async function migrateCloudSQLToNewDataStructure() {
  // Cloud SQL connection via proxy (more secure)
  const dataSource = new DataSource({
    type: 'postgres',
    host: 'localhost', // Proxy connection
    port: 5433, // Proxy port
    username: 'postgres',
    password: process.env.CLOUD_SQL_PASSWORD || 'your-cloud-sql-password', // Set this environment variable
    database: 'db',
    synchronize: false,
    logging: true,
  });
  ...
  try {
    console.log('🔗 Connecting to Cloud SQL database via proxy...');
    ...
    console.log('🧹 Cleaning old data from Cloud SQL...');
    ...
    console.log('✅ Old data cleaned from Cloud SQL');
    ...
    console.log('🏗️ Creating new Portefeuilles in Cloud SQL...');
    ...
    console.log('👨‍👩‍👧‍👦 Creating new Familles Actifs in Cloud SQL...');
    ...
    console.log('👥 Creating new Groupes Actifs in Cloud SQL...');
    ...
    console.log('🏭 Creating sample Actifs in Cloud SQL...');
    let actifCount = 0;
    ...
    console.log(`✅ Created ${actifCount} sample Actifs in Cloud SQL`);

    console.log('🔍 Creating Inspection Types in Cloud SQL...');
    ...
    console.log('🔗 Creating Types Inspection Groupe relationships in Cloud SQL...');
    ...
    console.log(`✅ Created ${data.types_inspection_groupe.length} inspection-groupe relationships in Cloud SQL`);
...
migrateCloudSQLToNewDataStructure();
